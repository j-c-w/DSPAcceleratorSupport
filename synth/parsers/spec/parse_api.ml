...
let load_alignment_map json =
	...
	match json with
	| `Assoc(_) ->
		let _ = List.map (json |> keys) ~f:(fun key ->
			let _ = Hashtbl.add table ~key:key ~data:(json |> member key |> to_int) in
			()
		) in
		table
	| `Null ->
...
let load_target_api options filename =
	...
	let livein = List.map (json |> member "livein" |> to_list) ~f:to_string in
	let liveout = List.map (json |> member "liveout" |> to_list) ~f:to_string in
	let typemap = load_typemap options json (livein @ liveout) in
	let funname = json |> member "functionname" |> to_string in
	let funargs = List.map (json |> member "functionargs" |> to_list) ~f:to_string in
