...
let extract_typemap options typemap vars =
	...
	let typepairs = List.map vars ~f:(fun var ->
	...
	ignore(List.map typepairs ~f:(fun (var, t)  -> Hashtbl.add tbl ~key:var ~data:t));
	tbl;;
...
let check_has_not json values =
	ignore(List.map values ~f:(fun m ->
...
let load_iospec options filename =
	...
	let livein = List.map (json |> member "livein" |> to_list) ~f:(fun j -> j |> to_string) in
	let liveout = List.map (json |> member "liveout" |> to_list) ~f:(fun j -> j |> to_string) in
	...
	let funargs = List.map (json |> member "funargs" |> to_list) ~f:(fun j -> j |> to_string) in
	let compiler_flags = match json |> member "compiler_flags" with
    | `Null -> []
    | other -> List.map (other |> to_list) ~f:to_string in
	let required_includes = List.map (json |> member "required_includes" |> to_list) ~f:to_string in
	let value_profiles = List.map (match json |> member "value_profiles" with
	| `Null -> []
	| other -> other |> to_list
	) ~f:load_value_profiles in
