...
Meow_FFT_Complex output_temp_5 = wn[i4];
json output_temp_6;
...
Meow_FFT_Complex output_temp_9 = wn_ordered[i8];
json output_temp_10;
...
output_temp_11["count"] = stages.count;
std::vector<json> output_temp_12;
for (unsigned int i13 = 0; i13 < stages.count; i13++) {
unsigned int output_temp_14 = radix[i13];

output_temp_12.push_back(stages.output_temp_14);
}
output_temp_11["radix"] = output_temp_12;
std::vector<json> output_temp_15;
for (unsigned int i16 = 0; i16 < stages.count; i16++) {
unsigned int output_temp_17 = remainder[i16];

output_temp_15.push_back(stages.output_temp_17);
}
output_temp_11["remainder"] = output_temp_15;
std::vector<json> output_temp_18;
for (unsigned int i19 = 0; i19 < stages.count; i19++) {
unsigned int output_temp_20 = offsets[i19];

output_temp_18.push_back(stages.output_temp_20);
}
...
size_t workset_bytes = meow_fft_generate_workset_complex(n, NULL);
Meow_FFT_Workset_Complex fft_complex =
	(Meow_FFT_Workset_Complex *) malloc(workset_bytes);
meow_fft_generate_workset_complex(n, fft_complex);
Meow_FFT_Complex *in = &in_vec[0];
Meow_FFT_Complex out[n];
clock_t begin = clock();
meow_fft_complex(data, in, out);
clock_t end = clock();
